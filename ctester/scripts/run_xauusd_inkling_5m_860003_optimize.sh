#!/bin/bash
# Script to run XAUUSD Inkling 5M 860003 Strategy Optimization
# Cleans ctester folder and saves optimization results to tmp folder
#
# Usage: ./run_xauusd_inkling_5m_860003_optimize.sh [OPTIONS]
# Options:
...
# Configuration
CONFIG_FILE="config/Inkling_XAUUSD-5M_860003_optimize.config"
SYMBOL="XAUUSD"
STRATEGY_ID="860003"
OUTPUT_DIR="tmp"
...
if [ ! -f "$CONFIG_FILE" ]; then
    ...
    echo "  Falling back to regular config: config/Inkling_XAUUSD-5M_860003.config"
    CONFIG_FILE="config/Inkling_XAUUSD-5M_860003.config"
    if [ ! -f "$CONFIG_FILE" ]; then
...
echo "=== Running ${SYMBOL} Inkling 5M Strategy ${STRATEGY_ID} Optimization ==="
echo ""
...
echo "  - AUTOBBS_IS_ATREURO_RANGE: 5.0 to 15.0 (step 1.0)"
echo "  - AUTOBBS_RANGE: 0 to 1 (step 1)"
echo "  - Total combinations: 5000 (10 × 5 × 10 × 11 × 2)"
...
if [ "$OPTIMIZATION_SUCCESS" = true ]; then
    ...
    echo "  sort -t, -k12 -rn ${RESULTS_FOLDER}/optimization_results_${STRATEGY_ID}.csv | head -20"
else
