#!/bin/bash
# Script to run GBPJPY Inkling 5M 860002 Strategy Optimization
# Cleans ctester folder and saves optimization results to tmp folder
#
# Usage: ./run_gbpjpy_inkling_5m_860002_optimize.sh [OPTIONS]
# Options:
...
# Configuration
CONFIG_FILE="config/Inkling_GBPJPY-5M_860002_optimize.config"
SYMBOL="GBPJPY"
STRATEGY_ID="860002"
OUTPUT_DIR="tmp"
...
if [ ! -f "$CONFIG_FILE" ]; then
    ...
    echo "  Falling back to regular config: config/Inkling_GBPJPY-5M_860002.config"
    CONFIG_FILE="config/Inkling_GBPJPY-5M_860002.config"
    if [ ! -f "$CONFIG_FILE" ]; then
...
echo "  - AUTOBBS_IS_ATREURO_RANGE: 0.5 to 1.5 (step 0.1)"
echo "  - AUTOBBS_RANGE: 0.0 to 1.0 (step 1.0)"
echo "  - AUTOBBS_STARTHOUR: 14 to 18 (step 1)"
echo "  - Total combinations: Variable (Genetic Algorithm)"
