#!/bin/bash
# Script to run XAUUSD Shellington 4H Strategy 842001 Optimization
# Cleans ctester folder and saves optimization results to tmp folder
#
# Usage: ./run_xauusd_shellington_4h_842001_optimize.sh [OPTIONS]
# Options:
...
# Configuration
CONFIG_FILE="config/Bbs_XAUUSD-4H_Boduan_842001_optimize.config"
SYMBOL="XAUUSD"
STRATEGY_ID="842001"
OUTPUT_DIR="tmp"
...
if [ ! -f "$CONFIG_FILE" ]; then
    ...
    echo "  Falling back to regular config: config/Bbs_XAUUSD-1H_BoDuan_842001.config"
    CONFIG_FILE="config/Bbs_XAUUSD-1H_BoDuan_842001.config"
    if [ ! -f "$CONFIG_FILE" ]; then
...
echo "  - AUTOBBS_SHELLINGTON_TP_MULTIPLIER: 3.0 to 5.0 (step 0.5)"
echo "  - AUTOBBS_SHELLINGTON_BUY_WON_TIMES: 1 to 3 (step 1)"
