#!/bin/bash
# Script to run GBPJPY BBS BoDuan 4H Strategy 841005 Optimization
# Cleans ctester folder and saves optimization results to tmp folder
#
# Usage: ./run_gbpjpy_bbs_boduan_4h_841005_optimize.sh [OPTIONS]
# Options:
...
# Configuration
CONFIG_FILE="config/Bbs_GBPJPY-4H_Boduan_841005_optimize.config"
SYMBOL="GBPJPY"
STRATEGY_ID="841005"
OUTPUT_DIR="tmp"
...
if [ ! -f "$CONFIG_FILE" ]; then
    ...
    echo "  Falling back to regular config: config/Bbs_GBPJPY-4H_Boduan_841005.config"
...
echo "=== Running ${SYMBOL} BBS BoDuan 4H Strategy ${STRATEGY_ID} Optimization ==="
echo ""
echo "Optimization Parameters:"
echo "  - AUTOBBS_BBS_WEEKLY_ATR_FACTOR: 0.2 to 0.6 (step 0.1)"
echo "  - AUTOBBS_BBS_ENABLE_WEEKLY_ATR_VALIDATION: 0.0 to 1.0 (binary)"
