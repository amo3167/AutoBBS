#!/bin/bash
# Script to run XAUUSD Inkling 5M 860001 Strategy Optimization
# Cleans ctester folder and saves optimization results to tmp folder
#
# Usage: ./run_xauusd_inkling_5m_860001_optimize.sh [OPTIONS]
# Options:
...
# Configuration
CONFIG_FILE="config/Inkling_XAUUSD-5M_860001_optimize.config"
SYMBOL="XAUUSD"
STRATEGY_ID="860001"
OUTPUT_DIR="tmp"
...
if [ ! -f "$CONFIG_FILE" ]; then
    ...
    echo "  Falling back to regular config: config/Inkling_XAUUSD-5M_860001.config"
    CONFIG_FILE="config/Inkling_XAUUSD-5M_860001.config"
    if [ ! -f "$CONFIG_FILE" ]; then
