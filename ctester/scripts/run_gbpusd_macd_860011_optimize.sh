#!/bin/bash
# Script to run GBPUSD MACD 860011 Strategy Optimization
# Cleans ctester folder and saves optimization results to tmp folder
#
# Usage: ./run_gbpusd_macd_860011_optimize.sh [OPTIONS]
# Options:
...
# Configuration
CONFIG_FILE="config/Peso_MACD_GBPUSD-1H_860011_optimize.config"
SYMBOL="GBPUSD"
STRATEGY_ID="860011"
OUTPUT_DIR="tmp"
...
if [ ! -f "$CONFIG_FILE" ]; then
    ...
    echo "  Falling back to regular config: config/Peso_MACD_GBPUSD-1H.config"
    CONFIG_FILE="config/Peso_MACD_GBPUSD-1H.config"
    if [ ! -f "$CONFIG_FILE" ]; then
...
echo "  - AUTOBBS_RISK_CAP: 0.0 to 4.0 (step 0.5)"
echo "  - AUTOBBS_STOP_MOVEBACK_SL: 0.0 to 1.0 (step 1.0)"
