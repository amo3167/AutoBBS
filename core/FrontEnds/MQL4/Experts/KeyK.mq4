...
extern string FRAMEWORK_CONFIG      = "C:\\Users\\amo31\\AppData\\Roaming\\MetaQuotes\\Terminal\\3294B546D50FEEDA6BF3CFC7CF858DB7\\MQL4\\Files\\AsirikuyConfig.xml";
...
extern int    STRATEGY_INSTANCE_ID  = 550001;     // Unique identifier for an instance of internal strategy 1.
...
extern int    MAX_OPEN_ORDERS       = 20;      // The maximum number of orders the instance is allowed to have open at a time.
extern int    ATR_AVERAGING_PERIOD  = 20;
   
extern string SECTION_3             = "############## Additional Strategy Settings ##############";
extern int    KK_SL_MODE            = 1;      // Trader set the daily trend. 1= UP, -1= Down, 0= Range. If strong trend, you can add more risk by increasing ACCOUNT_RISK_PERCENT
extern double KK_MULTIPLE_ATR       = 0.5;     
extern double KK_ADJUSTPOINTS       = 0.0005;   // Add this adjusted points on stop loss and take profit. It can be caculated from Primary ATR? Not sure which one is better?
...
bool initialize(double& systemSettings[][SYSTEM_SETTINGS_ARRAY_SIZE], double& strategySettings[][STRATEGY_SETTINGS_ARRAY_SIZE], charArray& strategyStrings[][STRATEGY_STRINGS_ARRAY_SIZE], double& profitDrawdown[][PROFIT_DRAWDOWN_ARRAY_SIZE], charArray& ratesSymbols[][TOTAL_RATES_ARRAYS], double& ratesInformation[][TOTAL_RATES_ARRAYS][RATES_INFO_ARRAY_SIZE])
{
  ...
  strategySettings[0][IDX_INTERNAL_STRATEGY_ID]  = KEYK;
  ...
  strategySettings[0][IDX_ADDITIONAL_PARAM_1]    = KK_SL_MODE;
  strategySettings[0][IDX_ADDITIONAL_PARAM_2]    = KK_MULTIPLE_ATR;
  strategySettings[0][IDX_ADDITIONAL_PARAM_3]    = KK_ADJUSTPOINTS;  
  ...
  ratesInformation[0][0][IDX_TOTAL_BARS_REQUIRED]= 300;
  ...
  ratesInformation[0][1][IDX_TOTAL_BARS_REQUIRED]= 25;
