...
extern string FRAMEWORK_CONFIG      = "C:\\Users\\amo31\\AppData\\Roaming\\MetaQuotes\\Terminal\\188ED535E9CD0077B15997DB4A8036D2\\MQL4\\Files\\AsirikuyConfig.xml";
...
extern int    STRATEGY_MODE       = 1;      // DISABLE = 0, ENABLE = 1, MONITOR = 2. In MONITOR mode current orders are managed but no new orders are opened.
extern bool   RUN_EVERY_TICK        = false;  // If this is set to false the strategy will only run once per bar.
extern int    STRATEGY_INSTANCE_ID  = 66666;     // Unique identifier for an instance of internal strategy 1.
extern int    STRATEGY_TIMEFRAME    = 15;      // 1hour = 60, 4hour = 240, daily = 1440.
extern double ACCOUNT_RISK_PERCENT  = 0.5;    // Percentage of account risked per trade. Warning: Losses will sometimes exceed this value!
...
extern double MAX_SPREAD_PIPS       = 0.05;    // Max spread in pips. Trades will not be opened when the spread is wider than this value.
...
extern int    MAX_OPEN_ORDERS       = 6;      // The maximum number of orders the instance is allowed to have open at a time.
extern int    ATR_AVERAGING_PERIOD  = 20;
extern int    MAX_ORDERS_IN_ACCOUNT = 100;  // Max open orders in the whole account
   
extern string SECTION_3             = "############## Additional Strategy Settings ##############";
extern int    BBS_PERIOD            = 20;     
extern int    BBS_DEVIATION         = 2;
extern double BBS_ADJUSTPOINTS      = 0.05;  // Add this adjusted points on stop loss and take profit. It can be caculated from Primary ATR? Not sure which one is better?
extern double POSITION              = 0;
extern int    DSL_TYPE              = 0;    // type of dynamic SL to use 0= none, 1= One day, 2= Two days, 3= 1H M200, 4=BBS
...
bool initialize(double& systemSettings[][SYSTEM_SETTINGS_ARRAY_SIZE], double& strategySettings[][STRATEGY_SETTINGS_ARRAY_SIZE], charArray& strategyStrings[][STRATEGY_STRINGS_ARRAY_SIZE], double& profitDrawdown[][PROFIT_DRAWDOWN_ARRAY_SIZE], charArray& ratesSymbols[][TOTAL_RATES_ARRAYS], double& ratesInformation[][TOTAL_RATES_ARRAYS][RATES_INFO_ARRAY_SIZE])
{
  ...
  strategySettings[0][IDX_INTERNAL_STRATEGY_ID]  = TAKEOVER;
  ...
  //If you want to support take over, need to load get open orders into strategy.
  strategySettings[0][IDX_ORDERINFO_ARRAY_SIZE]  = MAX_ORDERS_IN_ACCOUNT; 
  
  strategySettings[0][IDX_USE_SL]     			 = 0;
  strategySettings[0][IDX_USE_TP]     			 = 0;
  
   
  strategySettings[0][IDX_ADDITIONAL_PARAM_1]    = BBS_PERIOD;
  strategySettings[0][IDX_ADDITIONAL_PARAM_2]    = BBS_DEVIATION;
  strategySettings[0][IDX_ADDITIONAL_PARAM_3]    = BBS_ADJUSTPOINTS;  
  strategySettings[0][IDX_ADDITIONAL_PARAM_4]    = POSITION; 
  ...
  ratesInformation[0][0][IDX_TOTAL_BARS_REQUIRED]= 120;
  ...
  ratesInformation[0][1][IDX_TOTAL_BARS_REQUIRED]= 30;
