...
{
    /* Reduce risk on Wednesday for GBPUSD (typically higher volatility day) */
    ...
    int *signalMAPeriod, int *tradingDays, BOOL *shouldReturn)
{
    ...
    *startHour = START_HOUR_BTC_ETH;
    ...
    /* Reduce risk on weekdays (Tue-Thu) when institutional trading activity increases */
    if (timeInfo->tm_wday == TUESDAY_WDAY || timeInfo->tm_wday == WEDNESDAY_WDAY || timeInfo->tm_wday == THURSDAY_WDAY)
