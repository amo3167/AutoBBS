...
1. **Determine which vendors are actually needed:**
   - Check which projects in `dev/` actually depend on vendor libraries
   - Review include paths in `premake4.lua` to see which are referenced

2. **For TradingStrategies specifically:**
   - Check if `dev/TradingStrategies` depends on any vendor libraries
...
- Source code includes in `dev/` projects
