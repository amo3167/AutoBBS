...
struct SMTP {
  ...
  int trailing_crlf;      /* Specifies if the tailing CRLF is present */
};
...
struct smtp_conn {
  ...
  int ssldone;            /* Is connect() over SSL done? */
  char *domain;            /* Client address/name to send in the EHLO */
  unsigned int authmechs;  /* Accepted authentication mechanisms */
  unsigned int prefmech;   /* Preferred authentication mechanism */
  unsigned int authused;   /* Auth mechanism used for the connection */
  int tls_supported;      /* StartTLS capability supported by server */
  int size_supported;     /* If server supports SIZE extension according to
                              RFC 1870 */
};
