...
struct pingpong {
  ...
  char *linestart_resp; /* line start pointer for the server response
                           reader function */
  int pending_resp;  /* set TRUE when a server response is pending or in
  ...
  int (*endofresp)(struct connectdata *conn, char *ptr, size_t len,
                    int *code);
};
 ...
 */
CURLcode Curl_pp_statemach(struct pingpong *pp, int block);
 ...
 */
int Curl_pp_moredata(struct pingpong *pp);
