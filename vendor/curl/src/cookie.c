...
struct Cookies *cookie_getlist(struct CookieInfo *cookie,
                               char *host, char *path, int secure);

        For a given host and path, return a linked list of cookies that
        the client should send to the server if used now. The secure
        intean informs the cookie if a secure connection is achieved or
        not.
...
static int tailmatch(const char *cooke_domain, const char *hostname)
{
 ...
 */
static int pathmatch(const char* cookie_path, const char* request_uri)
{
                                       ...
                                       unless set */
{
  ...
  int replace_old = FALSE;
  int badcookie = FALSE; /* cookies are good by default. mmmmm yummy */
    ...
    for(ptr=firstptr, fields=0; ptr && !badcookie;
      ...
      case 2:
        ...
        if(strcmp("TRUE", ptr) && strcmp("FALSE", ptr)) {
          /* only if the path doesn't look like a intean option! */
          co->path = strdup(ptr);
          if(!co->path)
            badcookie = TRUE;
          else {
 ...
 * client should send to the server if used now. The secure intean informs
...
void Curl_cookie_freelist(struct Cookie *co, int cookiestoo)
{
  struct Cookie *next;
  if(co) {
    while(co) {
