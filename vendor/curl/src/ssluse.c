...
static int rand_enough(void)
{
  return (0 != RAND_status()) ? TRUE : FALSE;
}
#else
#define seed_enough(x) rand_enough(x)
static int rand_enough(int nread)
{
...
{
  /* we have the "SSL is seeded" intean static to prevent multiple
     time-consuming seedings in vain */
  static int ssl_seeded = FALSE;
...
int Curl_ossl_data_pending(const struct connectdata *conn,
                            int connindex)
{
