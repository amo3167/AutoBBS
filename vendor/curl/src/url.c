...
static void signalPipeClose(struct curl_llist *pipeline, int pipe_broke);
                     ...
                     va_list param)
{
  ...
  case CURLOPT_SSL_VERIFYHOST:
    ...
    /* Obviously people are not reading documentation and too many thought
       this argument took a intean when it wasn't and misused it. We thus ban
...
CURLcode Curl_disconnect(struct connectdata *conn, int dead_connection)
{
  struct SessionHandle *data;
  if(!conn)
 ...
 */
static int SocketIsDead(curl_socket_t sock)
{
  int sval;
  int ret_val = TRUE;
...
static int IsPipeliningPossible(const struct SessionHandle *handle,
...
int Curl_isPipeliningEnabled(const struct SessionHandle *handle)
{
...
void Curl_getoff_all_pipelines(struct SessionHandle *data,
                               struct connectdata *conn)
{
  int recv_head = (conn->readchannel_inuse &&
    (gethandleathead(conn->recv_pipe) == data)) ? TRUE : FALSE;

  int send_head = (conn->writechannel_inuse &&
                 ...
                 int *force_reuse)
{
  ...
  int canPipeline = IsPipeliningPossible(data, needle);
  int wantNTLM = (data->state.authhost.want==CURLAUTH_NTLM) ||
...
CURLcode Curl_protocol_doing(struct connectdata *conn, int *done)
{
  CURLcode result=CURLE_OK;
 ...
 */
static int tld_check_name(struct SessionHandle *data,
...
****************************************************************/
static int check_noproxy(const char* name, const char* no_proxy)
{
                            ...
                            int *async)
{
  ...
  int reuse;
  char *proxy = NULL;
  int prot_missing = FALSE;
  int no_connections_available = FALSE;
  int force_reuse;
...
CURLcode Curl_do(struct connectdata **connp, int *done)
{
  CURLcode result=CURLE_OK;
...
CURLcode Curl_do_more(struct connectdata *conn, int *completed)
{
  CURLcode result=CURLE_OK;
