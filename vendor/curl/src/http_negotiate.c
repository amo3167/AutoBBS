...
static int
get_gss_name(struct connectdata *conn, int proxy, gss_name_t *server)
{
...
int Curl_input_negotiate(struct connectdata *conn, int proxy,
                         const char *header)
{
...
CURLcode Curl_output_negotiate(struct connectdata *conn, int proxy)
{
  struct negotiatedata *neg_ctx = proxy?&conn->data->state.proxyneg:
    &conn->data->state.negotiate;
  char *encoded = NULL;
