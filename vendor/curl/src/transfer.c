 ...
 */
int Curl_meets_timecondition(struct SessionHandle *data, time_t timeofdoc)
{
...
static CURLcode readwrite_data(struct SessionHandle *data,
                               ...
                               int *didwhat, int *done)
{
  CURLcode result = CURLE_OK;
  ssize_t nread; /* number of bytes read */
  size_t excess = 0; /* excess bytes read */
  int is_empty_data = FALSE;
  int readmore = FALSE; /* used by RTP to signal for more data */
                        ...
                        int *done)
{
  ...
  /* Now update the "done" intean we return */
...
static size_t strlen_url(const char *url)
{
  const char *ptr;
  size_t newlen=0;
  int left=TRUE; /* left side of the ? */
