...
static CURLcode ftp_regular_transfer(struct connectdata *conn, int *done);
...
static CURLcode ftp_state_quote(struct connectdata *conn,
                                int init, ftpstate instate);
static CURLcode ftp_nb_type(struct connectdata *conn,
                            int ascii, ftpstate newstate);
static int ftp_need_type(struct connectdata *conn,
                         int ascii);
static CURLcode ftp_do(struct connectdata *conn, int *done);
static CURLcode ftp_done(struct connectdata *conn,
                         CURLcode, int premature);
static CURLcode ftp_connect(struct connectdata *conn, int *done);
static CURLcode ftp_disconnect(struct connectdata *conn, int dead_connection);
static CURLcode ftp_do_more(struct connectdata *conn, int *completed);
static CURLcode ftp_multi_statemach(struct connectdata *conn, int *done);
 ...
 *
 */
static CURLcode ReceivedServerConnect(struct connectdata *conn, int *received)
{
  struct SessionHandle *data = conn->data;
 ...
 *
 */
static CURLcode AllowServerConnect(struct connectdata *conn, int *connected)
{
  struct SessionHandle *data = conn->data;
  long timeout_ms;
...
static int ftp_endofresp(struct connectdata *conn, char *line, size_t len,
                          int *code)
{
  (void)conn;
...
static CURLcode ftp_connect(struct connectdata *conn,
                                 int *done) /* see description above */
{
  CURLcode result;
  struct ftp_conn *ftpc = &conn->proto.ftpc;
  struct pingpong *pp = &ftpc->pp;
                              ...
                              int premature)
{
  ...
  int ftpcode;
  CURLcode result = CURLE_OK;
  int was_ctl_valid = ftpc->ctl_valid;
  char *path;
...
static CURLcode ftp_do_more(struct connectdata *conn, int *complete)
{
  struct SessionHandle *data=conn->data;
  struct ftp_conn *ftpc = &conn->proto.ftpc;
  CURLcode result = CURLE_OK;
...
CURLcode ftp_perform(struct connectdata *conn,
                     int *connected,  /* connect status after PASV / PORT */
                     int *dophase_done)
{
