...
static int isHandleAtHead(struct SessionHandle *handle,
                                   ...
                                   CURL *curl_handle)
{
  ...
  if(easy) {
    int premature = (easy->state < CURLM_STATE_COMPLETED) ? TRUE : FALSE;
    int easy_owns_conn = (easy->easy_conn &&
...
int Curl_multi_pipeline_enabled(const struct Curl_multi *multi)
{
                                 ...
                                 struct Curl_one_easy *easy)
{
  ...
  int connected;
  int async;
  int protocol_connect = FALSE;
  int dophase_done = FALSE;
  int done = FALSE;
