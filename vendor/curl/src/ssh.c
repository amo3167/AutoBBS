...
static CURLcode ssh_connect(struct connectdata *conn, int *done);
static CURLcode ssh_multi_statemach(struct connectdata *conn, int *done);
static CURLcode ssh_do(struct connectdata *conn, int *done);
...
static CURLcode scp_doing(struct connectdata *conn,
                          int *dophase_done);
static CURLcode scp_disconnect(struct connectdata *conn, int dead_connection);

static CURLcode sftp_done(struct connectdata *conn,
                          CURLcode, int premature);
static CURLcode sftp_doing(struct connectdata *conn,
                           int *dophase_done);
static CURLcode sftp_disconnect(struct connectdata *conn, int dead);
static
...
static CURLcode ssh_statemach_act(struct connectdata *conn, int *block)
{
  CURLcode result = CURLE_OK;
  struct SessionHandle *data = conn->data;
  ...
  do {
    ...
    case SSH_AUTH_PKEY_INIT:
      ...
      if((data->set.ssh_auth_types & CURLSSH_AUTH_PUBLICKEY) &&
        ...
        char *home = NULL;
        int rsa_pub_empty_but_ok = FALSE;
 ...
 */
static void ssh_block2waitfor(struct connectdata *conn, int block)
{
  struct ssh_conn *sshc = &conn->proto.sshc;
  int dir;
  if(!block)
...
static CURLcode ssh_multi_statemach(struct connectdata *conn, int *done)
{
  struct ssh_conn *sshc = &conn->proto.sshc;
  CURLcode result = CURLE_OK;
  int block; /* we store the status and use that to provide a ssh_getsock()
                 implementation */
...
/* BLOCKING, but the function is using the state machine so the only reason
   this is still blocking is that the multi interface code has no support for
   disconnecting operations that takes a while */
static CURLcode sftp_disconnect(struct connectdata *conn, int dead_connection)
{
  CURLcode result = CURLE_OK;
  (void) dead_connection;
