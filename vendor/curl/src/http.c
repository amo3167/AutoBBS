...
#ifdef USE_SSL
static CURLcode https_connecting(struct connectdata *conn, int *done);
 ...
 * Returns CURLcode.
 */
static CURLcode http_output_basic(struct connectdata *conn, int proxy)
{
  size_t size = 0;
 ...
 */
static int pickoneauth(struct auth *pick)
{
  int picked;
 ...
 * @param proxytunnel intean if this is the request setting up a "proxy
 * tunnel"
 *
 * @returns CURLcode
 */
...
Curl_http_output_auth(struct connectdata *conn,
                      const char *request,
                      const char *path,
                      int proxytunnel) /* TRUE if this is the request setting
                                           up the proxy tunnel */
{
  CURLcode result = CURLE_OK;
  struct SessionHandle *data = conn->data;
      ...
      *availp |= CURLAUTH_GSSNEGOTIATE;
      ...
      if(authp->picked == CURLAUTH_GSSNEGOTIATE) {
        if(data->state.negotiate.state == GSS_AUTHSENT) {
          ...
          neg = Curl_input_negotiate(conn, (int)(httpcode == 407), start);
          if(neg == 0) {
 ...
 */
CURLcode Curl_http_connect(struct connectdata *conn, int *done)
{
  CURLcode result;
...
static int use_http_1_1(const struct SessionHandle *data,
 ...
 * HTTP request.
 */
CURLcode Curl_http(struct connectdata *conn, int *done)
{
  struct SessionHandle *data=conn->data;
  CURLcode result=CURLE_OK;
  struct HTTP *http;
  const char *ppath = data->state.path;
  int paste_ftp_userpwd = FALSE;
