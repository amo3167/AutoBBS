...
/* Local API functions */
static CURLcode imap_regular_transfer(struct connectdata *conn, int *done);
static CURLcode imap_do(struct connectdata *conn, int *done);
static CURLcode imap_done(struct connectdata *conn, CURLcode status,
                          int premature);
static CURLcode imap_connect(struct connectdata *conn, int *done);
static CURLcode imap_disconnect(struct connectdata *conn, int dead);
static CURLcode imap_multi_statemach(struct connectdata *conn, int *done);
static int imap_getsock(struct connectdata *conn, curl_socket_t *socks,
                        int numsocks);
static CURLcode imap_doing(struct connectdata *conn, int *dophase_done);
 ...
 */
static int imap_matchresp(const char *line, size_t len, const char *cmd)
{
 ...
 */
static int imap_endofresp(struct connectdata *conn, char *line, size_t len,
                           int *resp)
{
 ...
 */
static CURLcode imap_perform(struct connectdata *conn, int *connected,
                             int *dophase_done)
{
 ...
 * resources. BLOCKING.
 */
static CURLcode imap_disconnect(struct connectdata *conn, int dead_connection)
{
  struct imap_conn *imapc = &conn->proto.imapc;
...
static CURLcode imap_dophase_done(struct connectdata *conn, int connected)
{
  struct IMAP *imap = conn->data->state.proto.imap;
 ...
 * grammar of RFC-5092.
 */
static int imap_is_bchar(char ch)
{
  switch(ch) {
    /* bchar */
    case ':': case '@': case '/':
