...
public class FileService implements IFileService {
	...
	public FileService(IModelData model,IStatisticsService service) {
		this.model = model;
	...
	public List<Results> readClosedOrdersCSV(String filePath) {
		...
		try (Reader reader = Files.newBufferedReader(Paths.get(filePath));
			...
			List<String[]> records = csvReader.readAll();
			return records.stream().map(FileService::convertClosedResults).collect(Collectors.toList());
	...
	public List<Results> readOpenOrdersCSV(String filePath) {
		...
		try (Reader reader = Files.newBufferedReader(Paths.get(filePath));
			...
			List<String[]> records = csvReader.readAll();
			return records.stream().map(FileService::convertOpenResults).collect(Collectors.toList());
...
//                	continue;
	...
	public void generateWeelyReport(String filePath, boolean isAdjusted) throws IOException {
		try (Writer writer = Files.newBufferedWriter(Paths.get(
			...
			Double balance = ModelData.INITBALANCE;
